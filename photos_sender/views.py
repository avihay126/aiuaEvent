...
def send_photos(request):
    return HttpResponse("This is the photo sending page")
            ...
            for selfie in event_selfies:
                if id_guest.is_same_person(selfie.get_encoding()):
                    ...
                    for img in unsent_images:
                        ...
                        img.sent = True
                        img.save()
                    test_image_upload(paths, selfie.guest, driver)
        time.sleep(3)
    driver.quit()
...
def test_image_upload(image_paths, guest, driver):
    ...
    attach_btn = WebDriverWait(driver, 30).until(
        EC.presence_of_element_located((By.XPATH, '//*[@id="main"]/footer/div[1]/div/span[2]/div/div[1]/div[2]/div/div'))
    )
    time.sleep(2)
    attach_btn.click()
    time.sleep(2)
